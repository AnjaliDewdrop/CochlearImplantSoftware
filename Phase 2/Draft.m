...
butterWorthChannelResult = separater(@ButterWorth, channel, yResamp);
windowFIRChannelResult = separater(@ButterWorth, channel, yResamp);
gEquiFIRChannelResult = separater(@ButterWorth, channel, yResamp);
chebyshevlIIRChannelResult = separater(@ButterWorth, channel, yResamp);

plotFilteredAudio(butterWorthChannelResult)

function plotFilteredAudio(channelResult)
for channelNum=1:length(channelResult)
    figure("NumberTitle",channelNum,"Name",strcat("Channel",num2str(channelNum)))
    plot(tResamp, channelResult{channelNum});
    xlabel('Time(s)')
    ylabel('Amplitude')
    %     tFiltered=1/16000:1/16000:size(channelResult{channelNum})/16000;
%     yCosFiltered=cos(2*pi*1000*tFiltered);
%     plot(tFiltered(1:500),yCosFiltered(1:500))
%     figure
%     plot(tResamp(1:250),channelResult{channelNum}(1:250));
end
end
...
function out = separater(func,channel,y)
out = {};
for range =1:length(channel)
    out = [out, filter(func(channel{range}),y)];
end
end
